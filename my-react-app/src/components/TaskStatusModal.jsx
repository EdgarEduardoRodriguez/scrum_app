...
export function TaskStatusModal({ task, isOpen, onClose, onSave, onLogTime }) {
  ...
  const safeEstimatedHours = typeof task.estimatedHours === "number" ? task.estimatedHours : 0;

  const totalHoursLogged = safeTimeEntries.reduce((sum, entry) => sum + (entry?.hours ?? 0), 0);
  const remainingHours = safeEstimatedHours - totalHoursLogged;
  const progressPercentage = safeEstimatedHours > 0
    ? Math.min((totalHoursLogged / safeEstimatedHours) * 100, 100)
    : 0;
...
// The rest of the app (e.g. TaskCard.jsx) imports this component as a default export.
// Providing a default export prevents runtime errors like:
// "does not provide an export named 'default'" which can cause a blank page.
